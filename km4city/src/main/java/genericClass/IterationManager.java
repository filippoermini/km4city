...
public class IterationManager {
	...
	private void init(IterationElement it){
		Iterator<Class> itc = this.origin.getClazz().iterator();
		Tree.Class c;
		while(itc.hasNext()){
			c = itc.next();
			GenericClass g = new GenericClass(c);
			it.getClasses().add(g);
		}
	...
	public void setID(String id){
		...
		{
			GenericClass g = it.getClasses().get(i);
			if (g.isRoot()){
				g.setID(id);
			}	
		}
	...
	private void _process(GenericClass g){
				...
				if(!(a.isPrimaryKey() && g.isRoot())){//non devo generare il valore per il campo id della clsse root, è già settato
					if(a.isValueExpression()){
						...
						if(a.isExternalKey()){//l'attributo è un riferimento ad un'altra classe
							...
							GenericClass refClass = it.getObjectClassByName(a.getType());
